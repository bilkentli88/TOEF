...
def save_current_results(out_dir, results_log, record_split_rows=None, final=False):
    ...
    write_csv(out_dir / "mitbih_trust_profiles_raw.csv", raw_rows, raw_fields)
    ...
    if summary_rows:
        ...
        write_csv(out_dir / "mitbih_trust_profiles_summary.csv", summary_rows, summary_fields)

    if record_split_rows is not None:
        split_fields = ["Seed", "Split", "Records"]
        write_csv(out_dir / "mitbih_record_splits.csv", record_split_rows, split_fields)
...
def build_latex_table(results_log):
    ...
    lines.append(r"\caption{Results (Mean $\pm$ Std over 3 seeds)}")
...
for i, seed in enumerate(cfg.seeds):
    ...
    # 1. Load Data
    X_tr, y_tr, X_te, y_te, train_recs, test_recs = load_mitbih_data(seed)
...
with open(out_dir / "mitbih_table4_latex.txt", "w", encoding="utf-8") as f:
    f.write(latex_table)

save_current_results(out_dir, results_log, record_split_rows=record_split_rows, final=True)

print("\nFINAL OUTPUT FILES")
print("-", out_dir / "mitbih_trust_profiles_raw.csv")
print("-", out_dir / "mitbih_trust_profiles_summary.csv")
print("-", out_dir / "mitbih_record_splits.csv")
print("-", out_dir / "run_config.json")
print("-", out_dir / "environment.json")
print("-", out_dir / "mitbih_table4_latex.txt")
